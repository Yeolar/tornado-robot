...
def fork_processes(num_processes, max_restarts=100, include_master=True):
    ...
    while children:
        ...
        id = children.pop(pid)
        if id == _master_id:
            logging.error("master child %d (pid %d) killed or exited", id, pid)
            for pid in children:
                logging.info("kill child %d (pid %d)", children[pid], pid)
                os.kill(pid, 9)
            sys.exit(1)
...
def master_id():
    """Returns the master task id.

    Returns None if this process was not created including master.
    """
    global _master_id
    return _master_id
